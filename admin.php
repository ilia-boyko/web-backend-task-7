...
if($_SERVER['REQUEST_METHOD']=='GET'){
  ...
  print('Вы успешно авторизовались и видите защищенные паролем данные. Также вы можете изменить данные или удалить пользлвателя');
  $users=array();
  $pwrs=array();
  $pwr_def=array('immortal','noclip','power','telepat');
  $pwrs_count=array();
  try{
    $get=$db->prepare("select * from application");
    $get->execute();
    $inf=$get->fetchALL();
    $get2=$db->prepare("select per_id,name from superp");
    $get2->execute();
    $inf2=$get2->fetchALL();
    $count=$db->prepare("select count(*) from superp where name=?");
